...
class CustomState(TypedDict):
    ...
    # Remove manual state management fields
    # LangGraph will handle checkpointing automatically
...
async def call_model(state: CustomState, tools):
    ...
    try:
        ...
        if total_length > 600000:  # Rough character limit (150k tokens * 4 chars/token)
            ...
            if recent_messages:
                # Keep last 6 messages (3 human-AI pairs) to preserve recent context
                keep_count = min(6, len(recent_messages))
...
def tools_router(state: CustomState):
    ...
    return "tools" if tool_calls else "wait_for_approval"  # Changed node name


def wait_for_approval(state: CustomState):
    """Node that interrupts for human approval using Command pattern"""
    print(f"DEBUG [wait_for_approval]: ENTERING - iteration: {state.get('iteration_count', 0)}")
    ...
    # Return Command for interrupt with state update
    return Command(
        update={
            "review_data": {
                ...
                "user_notes": state.get("user_notes", ""),
                "origin_tab": state.get("review_data", {}).get("origin_tab", "tab1")
            },
            "approval_status": "pending"
        },
        goto="interrupt"  # This triggers the interrupt
    )
...
async def run_agent(prompt, rag_enabled=None, user_notes=None):
    """
    Run the agent graph for a single prompt with HIL support using Command pattern.
    """
    # Resolve RAG toggle
    print("*****************Entering into runagent*************")
    ...
    MAX_USER_REVISIONS = 3  # Define iteration limit
    ...
    # Route LLM to tools OR wait_for_approval
    ...
    # After wait_for_approval, branch based on approval_status
    builder.add_conditional_edges(
        "wait_for_approval",
    ...
    # Rejected routing with iteration limit
    def rejected_router(state):
        iteration = state.get("iteration_count", 0)
        if iteration >= MAX_USER_REVISIONS:
            return "approved"
        return "LLM_Call_with_Tool"
    ...
    # Compile with interrupt_before for proper checkpointing
    memory = MemorySaver()
    app = builder.compile(
        checkpointer=memory,
    ...
    # Use astream for proper interrupt handling
    config_with_recursion = {"recursion_limit": 20, **config}
    final_state = None

    try:
        async for event in app.astream(initial_state, config_with_recursion):
            # Check for interrupt events
            if hasattr(event, "checkpoint") and event.checkpoint:
                checkpoint_state = getattr(event.checkpoint, "values", {})
                if checkpoint_state.get("approval_status") == "pending":
                    final_state = checkpoint_state
                    break

            # Check for completion
            if hasattr(event, "end") and event.end:
                final_state = getattr(event.end, "values", {})
                break

        return final_state or initial_state

    except Exception as e:
        logging.error(f"Error in agent stream: {str(e)}", exc_info=True)
        raise
...
async def process_query(prompt: str, action: str = None):
    ...
    try:
        ...
        print("*********message sent to agent flow********", prompt)
        ...
        # Check if response contains approval_status for interrupt
...
def update_tab1_history(assistant_content, user_query):
    """Update tab1 chat history - maintains conversation context for AI Research Assistant"""
    if not assistant_content:
        return

    tab1_messages = st.session_state.get("tab1_messages", []) or []

    # Add user message if provided and not already present
    if user_query and not any(
        ...
        print(f"DEBUG: Added user message to tab1 history: {user_query[:50]}...")

    # Add assistant response if not already present
    if not any(
            isinstance(msg, dict) and msg.get("role") == "assistant" and msg.get("content") == assistant_content
            for msg in tab1_messages
    ):
        tab1_messages.append({"role": "assistant", "content": assistant_content})
        print(f"DEBUG: Added assistant message to tab1 history: {assistant_content[:50]}...")

    # Keep only last 4 messages (2 exchanges) to maintain context without bloating
    if len(tab1_messages) > 4:
        tab1_messages = tab1_messages[-4:]
        print("DEBUG: Trimmed tab1 history to last 4 messages")

    st.session_state["tab1_messages"] = tab1_messages
    print(f"DEBUG: Tab1 history updated. Total messages: {len(tab1_messages)}")


def extract_content_from_state(state):
    """Extract content from state for display - handles various message formats"""
    if not state or not isinstance(state, dict):
        return ""

    messages = state.get("messages", [])

    # Find the last assistant/AI message content
    for msg in reversed(messages):
        # Handle BaseMessage objects (LangChain)
        if isinstance(msg, (BaseMessage, AIMessage)):
    ...
    # Fallback: return content from last message
    if messages:
        last_msg = messages[-1]
        if isinstance(last_msg, (BaseMessage, AIMessage)):
...
def cleanup_hil_state():
    """Clean up HIL-related session state after processing"""
    keys_to_remove = [
        "approval_status", "review_data", "show_overlay_during_hil",
        "hil_processing_status", "hil_user_notes", "user_notes",
        "graph_origin_tab", "hil_origin_tab"
    ]
    ...
    print("DEBUG: Cleaned up HIL state")


def process_final_response(final_state, origin_tab):
    """Process final response from agent - handles both tab1 and tab2 appropriately"""
    if not final_state:
        print(f"DEBUG: No final state received for {origin_tab}")
        return

    # Store the complete response object
    st.session_state[f"last_response_{origin_tab}"] = final_state
    print(f"DEBUG: Stored response in last_response_{origin_tab}")

    # Extract content for display
    content = extract_content_from_state(final_state)
    if content:
        st.session_state[f"last_content_{origin_tab}"] = content
        print(f"DEBUG: Stored content in last_content_{origin_tab}")

        # Update chat history ONLY for tab1 (AI Research Assistant)
        if origin_tab == "tab1":
            user_query = st.session_state.get(f"last_query_{origin_tab}")
            update_tab1_history(content, user_query)
        else:
            print("DEBUG: Skipping history update for tab2 (Express Analysis)")
    else:
        print(f"DEBUG: No content extracted for {origin_tab}")

# Update the process_hil_decision function to handle user notes
async def process_hil_decision():
    """Process human review decision using Command pattern"""
    approval_status = st.session_state.get("hil_processing_status")
    user_notes = st.session_state.get("hil_user_notes", "")

    # Get origin tab from review data
    review_data = st.session_state.get("review_data") or {}
    origin_tab = review_data.get("origin_tab", "tab1")

    print(f"DEBUG: Processing HIL decision for {origin_tab}, status: {approval_status}")

    app = st.session_state.get("graph_app")
    cfg = st.session_state.get("graph_config") or {}

    if not app or not cfg:
        print("ERROR: No graph app or config found for HIL decision")
        st.session_state[f"last_content_{origin_tab}"] = "Error: Could not resume agent processing"
        cleanup_hil_state()
        return
    ...
    try:
        ...
        command = Command(
            update={
                ...
                "user_notes": user_notes if approval_status == "rejected" else ""
            },
            goto="rejected" if approval_status == "rejected" else "approved"
        )

        # Resume with astream
        config_with_recursion = {"recursion_limit": 20, **cfg}
        final_state = None

        async for event in app.astream(command, config_with_recursion):
            if hasattr(event, "checkpoint") and event.checkpoint:
                checkpoint_state = getattr(event.checkpoint, "values", {})
                if checkpoint_state.get("approval_status") == "pending":
                    final_state = checkpoint_state
                    print("DEBUG: Another approval needed - back to human review")
                    break
        ...
        # Handle the result
        if final_state and final_state.get("approval_status") == "pending":
            # Another human review needed
            st.session_state["approval_status"] = "pending"
            st.session_state["review_data"] = final_state.get("review_data")
            # Preserve origin tab in the new review data
            if "review_data" in st.session_state and st.session_state["review_data"]:
                st.session_state["review_data"]["origin_tab"] = origin_tab
            print("DEBUG: Back to human review cycle")
        else:
            # Process final response for the specific tab
            process_final_response(final_state, origin_tab)
            print(f"DEBUG: Final response processed for {origin_tab}")

    except Exception as e:
        logging.exception("Failed to process HIL decision")
        st.session_state[f"last_content_{origin_tab}"] = f"Error: {str(e)}"
        print(f"ERROR: HIL processing failed for {origin_tab}: {str(e)}")

    # Always clean up HIL state
    cleanup_hil_state()

    # Preserve the current tab before rerun
    st.session_state["preserve_tab"] = origin_tab
    print(f"DEBUG: Preserving tab state: {origin_tab}")
...
def main_app():
    ...
    if st.sidebar.checkbox("🔍 Debug Mode", False):
        ...
        st.sidebar.write(f"User notes in session: {st.session_state.get('user_notes', 'None')}")
        st.sidebar.write(f"HIL user notes: {st.session_state.get('hil_user_notes', 'None')}")
        st.sidebar.write(f"Approval status: {st.session_state.get('approval_status', 'None')}")
