...
# Get session timeout from environment variable, with a default fallback

load_dotenv()
session_timeout = int(os.getenv('SESSION_TIMEOUT', '3'))
import time
...
def create_how_it_works_page():
    ...
    with tab3:
        ...
        <div style="margin-left: 20px;">
            <div style="display: flex; align-items: center; margin-bottom: 12px;">
                ...
                <div>
                    <strong>MCP Server Integration</strong>: Leverages Model Context Protocol for seamless interoperability between analytical models, data sources, and external tools, creating a unified AI ecosystem.
                </div>
            </div>
            <div style="display: flex; align-items: center; margin-bottom: 12px;">
                <span style="font-size: 1.2em; margin-right: 10px;">☁️</span>
                <div>
                    <strong>Cloud Deployment</strong>: Hosted on AWS ECS Fargate for enterprise-grade scalability, reliability, and automatic resource management with zero server maintenance overhead.
                </div>
            </div>
            <div style="display: flex; align-items: center; margin-bottom: 12px;">
                <span style="font-size: 1.2em; margin-right: 10px;">⚡</span>
                <div>
                    <strong>Intelligent Caching</strong>: Advanced multi-layer caching system that strategically stores static financial data to deliver insights 3x faster than traditional approaches.
                </div>
            </div>
            <div style="display: flex; align-items: center; margin-bottom: 12px;">
                <span style="font-size: 1.2em; margin-right: 10px;">🔍</span>
                <div>
                    <strong>LangSmith Observability</strong>: Integrated LLM call tracing and debugging platform providing complete visibility into agent workflows, prompt engineering effectiveness, and LLM input/output analysis for continuous performance optimization.
                </div>
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col2:
        if st.button("🚀 Try AI Co-Pilot Now", type="primary", use_container_width=True, key="how_it_works_cta"):
            ...
            #
            # if active_sessions_count >= SessionManager._max_sessions:
            #     st.session_state.show_session_limit_error = True
            #     st.rerun()
            # else:
            #     # JUST proceed to login - session will be created AFTER successful auth
...
def create_demo_page():
    ...
    if st.session_state.expanded_response:
        ...
        if st.session_state.expanded_response == "tesla_forecast":
            with st.expander("Citi Group Holdings (C) 7-Year ARIMA Forecast Analysis", expanded=True):
                # Display the forecast chart - FIXED: use_container_width instead of use_column_width
                st.image("newplot.png", caption="Citi Group Holdings (C) 7-Year ARIMA Forecast",
                ...
                forecast_data = {
                    ...
                    "Forecasted Close": [97.8, 98.9, 100.2, 101.3, 102.5, 103.8, 105.0],
                    "95% CI Lower": [92.4, 91.5, 90.5, 89.0, 87.0, 85.2, 82.8],
                    "95% CI Upper": [103.2, 106.3, 110.0, 113.6, 118.0, 122.4, 127.2]
                }
                ...
                st.markdown("""
                ### 📈 ARIMA Forecast Analysis for Citi Group Holdings (C)

                Here is the ARIMA‐based forecast for Citi Group Holdings (C) through 2030.

                **Data & Preprocessing**
                • Daily closing data fetched from 1977–09 through 2025-09.
                • Log‐transform and first‐difference to achieve stationarity (Augmented Dickey‐Fuller p-value < 0.01 only after d = 1).

                **Optimal Model Selection**
                • Grid search p, d, q over [0…3]×{1}×[0…3], minimizing AIC.
                • Optimal: p = 2, d = 1, q = 1
                • AIC = 12,046.3

                **Model Summary (ARIMA(2,1,1))**
                Model: ARIMA(2,1,1) on log-prices
                Coefficients:
                • AR1 = 0.523 (SE 0.071)
                • AR2 = –0.134 (SE 0.065)
                • MA1 = –0.451 (SE 0.081)
                σ² (resid var) = 0.00075
                Log Likelihood = 6,020.2
                AIC = 12,046.3

                **Residual Diagnostics**
                • Box–Ljung (lags=12) χ²=10.4, p=0.26 → no remaining autocorrelation
                • Shapiro–Wilk on residuals: W=0.993, p=0.45 → no strong departure from normality
                • Residual mean ≈ 0, constant variance

                **Year-End Forecasts & 95% Confidence Intervals**
                (Exponential back-transform of log-price forecasts)

                **Commentary**
                Over the 2024–2030 horizon, ARIMA(2,1,1) projects a gradual upward drift in Citi's closing price, 
                rising from roughly $98 in 2024 to about $105 by 2030. The point path is mildly bullish but relatively 
                flat—implying modest growth rather than a sharp rally. Confidence bands widen over time, reflecting 
                increased long‐term uncertainty. In sum, the model suggests stability with a slight upward bias over 
                the next six years.

                **Recommendation:** 🟡 **HOLD** - The forecast suggests modest growth with increasing uncertainty over time. 
                Consider this analysis alongside fundamental factors and market conditions.
...
def create_about_page():
    ...
    with col2:
        if st.button("🚀 Start Your AI-Powered Research", type="primary", key="about_final_cta"):
            show_modal_spinner()  # <— this injects the spinner and yields control back to Streamlit
            st.session_state.show_login_modal = False
            st.session_state.show_login = True
            st.rerun()
            # active_sessions_count = SessionManager.get_active_sessions_count()
...
# Add the error page function
def show_session_limit_error():
    """Display a friendly session limit error page"""
    seo = SEOEntryPoints()
    seo.setup_page_config("landing")

    st.markdown("""
    <div class="hero-section">
        <h1 class="hero-title">Session Limit Reached 🚫</h1>
        <p class="hero-subtitle">We're experiencing high demand right now</p>
    </div>
    """, unsafe_allow_html=True)

    # Using Streamlit components instead of HTML
    st.info("😔 All concurrent sessions Are In Use")
    st.write("Our free tier currently supports **Limited concurrent sessions** to ensure quality service for all users.")
    ...
    with st.container():
        ...
        #st.markdown('<div class="session-limit-container">', unsafe_allow_html=True)
        st.subheader("What You Can Do:")
        st.markdown("### 🏖️ Time for a Mini Break!")

        st.markdown("""
            - 🎮 Why not play a quick game of desk tennis while you wait? 
              _(Just kidding, but a few minutes' break would do!)_

            - ⏰ Our system takes power naps after **15 minutes** of inactivity 
              _(It needs its beauty sleep too!)_

            - 🦉 Night owl? Try during off-peak hours when our servers are less busy
              _(They're introverts and prefer quieter times)_
        """)

        st.caption("PS: If you're seeing this message, our hamsters are probably taking their union-mandated break 🐹")

        st.markdown('</div>', unsafe_allow_html=True)

    st.success("**Thank you for your patience and understanding!**")

    # Add a button to go back home
    if st.button("← Return to Home", use_container_width=True):
        st.session_state.show_session_limit_error = False
        st.rerun()
...
def main():
    ...
    if 'show_session_limit_error' not in st.session_state:
        st.session_state.show_session_limit_error = False
    ...
    if current_time - st.session_state.last_cleanup > 60:
        SessionManager.cleanup_old_sessions()
        st.session_state.last_cleanup = current_time

    # Refresh active session on user activity
    if (st.session_state.get("authenticated", False) and
            st.session_state.get("session_id") and
            SessionManager.get_remaining_time(st.session_state.session_id,formatted=False) > 0):

        SessionManager.refresh_session(st.session_state.session_id)

    # Check if we need to show session limit error
    if st.session_state.show_session_limit_error:
        show_session_limit_error()
        return
    ...
    # with st.sidebar:
    #     # Auto-refresh sidebar every 5 seconds so MM:SS updates smoothly
    #     st_autorefresh(interval=5 * 1000, key="sidebar_refresh")
    #
    #     # Show Welcome, Logout, and Session Timer only if authenticated
    #     if st.session_state.get("authenticated", False) and st.session_state.get("username"):
    #         # Check session expiration first
    #         if st.session_state.get("session_id"):
    #             formatted_time, remaining_seconds = SessionManager.get_remaining_time(st.session_state.session_id)
    #             if remaining_seconds <= 0:
    #                 # Session expired, log them out immediately
    #                 from mcp_client.auth.auth_manager import AuthManager
    #                 SessionManager.remove_session(st.session_state.session_id)
    #                 st.session_state.session_id = None
    #                 AuthManager().logout()
    #                 st.stop()  # Prevent rendering rest of sidebar
    #
    #         # Only show welcome message and timer if session is still valid
    #         st.markdown(
    #             f"""
    #             <div style="
    #                 background: var(--bg-secondary);
    ...
    #         )
    #
    #         # Show session timer if session is active
    #         if st.session_state.get("session_id"):
    #             formatted_time, remaining_seconds = SessionManager.get_remaining_time(st.session_state.session_id)
    #
    #             if remaining_seconds > 0:
    #                 total_duration = 15 * 60  # 15 minutes in seconds
    #                 progress_percent = remaining_seconds / total_duration
    #                 st.progress(progress_percent)
    #                 st.info(f"⏰ Session time remaining: {formatted_time}")
    #             else:
    ...
    with st.sidebar:
        ...
        if st.session_state.get("authenticated", False) and st.session_state.get("username"):
            # Check session expiration first
            if st.session_state.get("session_id"):
                remaining_time = SessionManager.get_remaining_time(st.session_state.session_id,formatted=False)
                if remaining_time <= 0:
                    # Session expired, log them out immediately
                    from mcp_client.auth.auth_manager import AuthManager
                    SessionManager.remove_session(st.session_state.session_id)
                    st.session_state.session_id = None
                    AuthManager().logout()
                    # Use st.stop() to prevent rendering the rest of the sidebar content
                    st.stop()
            ...
            if st.session_state.get("session_id"):
                ...
                #remaining_time = SessionManager.get_remaining_time(st.session_state.session_id)
                formatted_time, remaining_seconds = SessionManager.get_remaining_time(st.session_state.session_id)

                # if remaining_time > 0:
                #     # Create a progress bar for visual indication
                #     progress_percent = (remaining_time / session_timeout) * 100  # 15 minutes total
                #     st.progress(progress_percent / 100)
                #     st.info(f"⏰ Session time remaining: {remaining_time} minutes")
                total_duration_secs = SessionManager._session_timeout
                progress_ratio = remaining_seconds / total_duration_secs if total_duration_secs > 0 else 0
                ...
                if remaining_seconds > 0:
                    # Custom progress bar
                    progress_html = f"""
                        <div style="width: 100%; background-color: #ddd; border-radius: 12px; height: 18px; margin-bottom: 6px;">
                            <div style="
                                width: {progress_ratio * 100:.1f}%;
                        ...
                        <div style="text-align:center; font-size:0.95rem; color:{bar_color}; font-weight:600;">
                            ⏰ Session time remaining: {formatted_time}
                        </div>
                        """
                ...
                # if remaining_seconds > 0:
                #     st.progress(remaining_seconds / (session_timeout * 60))
                #     st.info(f"⏰ Session time remaining: {formatted_time}")
                else:
        ...
        # NAVIGATION BUTTONS - MOVED OUTSIDE THE AUTHENTICATED CHECK
        def smooth_scroll_to_top():
            """JavaScript for smooth scrolling to top"""
            st.components.v1.html("""
            <script>
            ...
            """, height=0)
        # These should be visible to ALL users (logged in or not)
        st.markdown("### Navigation")
    ...
    # Move the smooth_scroll_to_top function outside the sidebar context


    # Rest of your main function (URL parameter handling, page routing, etc.)
    # ... [your existing code for URL handling and page routing]
